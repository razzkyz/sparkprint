...
export async function createPrintOrder(formData: FormData) {
  try {
    ...
    // Validate file sizes (max 6MB per file, max 30MB total for 5 images)
    ...
    for (const photoFile of photoFiles) {
      if (photoFile.size > MAX_FILE_SIZE) {
        return {
          error: `File ${photoFile.name} terlalu besar. Maksimal 6MB per file.`,
          status: 413
        };
      }
    ...
    if (totalSize > MAX_TOTAL_SIZE) {
      return {
        error: `Total ukuran file terlalu besar. Maksimal 30MB.`,
        status: 413
      };
    }
